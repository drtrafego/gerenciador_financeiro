...
export default function TransactionsClient({ rows, totalIncome, totalExpense }: Props) {
  const { hidden } = useValuesVisibility();

  const fmt = (value: number, currency: string = "BRL") =>
    hidden ? HIDDEN : formatCurrency(value, currency as Currency);

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white">Projetos e Avulsos</h1>
          <p className="text-sm text-zinc-400">Mentoria, consultoria, comissão e parcelas · {rows.length} lançamento(s)</p>
        </div>
        <Link
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-4">
          ...
          <p className={cn("text-lg font-bold", totalIncome - totalExpense >= 0 ? "text-white" : "text-red-400")}>
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-zinc-800">
              {rows.map(({ transaction, clientName, deleteAction }) => (
                <tr key={transaction.id} className="hover:bg-zinc-800/50 transition-colors">
                  ...
                  <td className={cn(
                    ...
                    {fmt(parseFloat(transaction.amount ?? "0"), transaction.currency ?? "BRL")}
                  </td>
                  <td className="px-4 py-3">
